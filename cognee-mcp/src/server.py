...
from cognee.infrastructure.databases.vector.embeddings.instruction_manager import (
    instruction_scope,
...
INSTRUCTION_CHOICES = ", ".join(list_instruction_types())


def _normalize_instruction_choice(value: str | None) -> str | None:
    if value is None:
        return None

    lowered = value.strip().lower()
    if lowered not in list_instruction_types():
        raise ValueError(
            f"Unsupported instruction_type {value!r}. Choose one of: {INSTRUCTION_CHOICES}."
        )
    return lowered
...
async def cognee_add_developer_rules(
    ...
    instruction_type : str, optional
        Instruction-aware embedding preset for this ingestion run. Use it to tell Cognee whether
        the content should be treated as question-answering (qa), natural-language-to-code (nl2code),
        code-to-code, code-to-comment, or completion seeds. Choose from: nl2code, qa, code2code, code2nl, code2completion.
        Leave blank to fall back to the default template defined by `EMBEDDING_INSTRUCTION_MODE`.
    ...
    instruction_type = _normalize_instruction_choice(instruction_type)
        ...
        with redirect_stdout(sys.stderr):
            ...
            try:
                await cognee_client.add(
                    file_path, node_set=["developer_rules"], instruction_type=instruction_type
                )
                ...
                await cognee_client.cognify(
                    graph_model=model, instruction_type=instruction_type
                )
...
async def cognify(
    ...
    instruction_type : str, optional
        Embedding instruction preset to use while embedding the raw documents and
        dataset graph. Select `nl2code` for natural-language questions over code,
        `qa` for standard document Q&A, `code2code` for refactors, `code2nl` for
        comment synthesis, or `code2completion` for autocompletion style corpora.
        Leave empty to fall back to the default environment configuration.
    ...
    async def cognify_task(
        ...
        with redirect_stdout(sys.stderr):
            ...
            await cognee_client.add(data, instruction_type=instruction_type)

            try:
                await cognee_client.cognify(
                    custom_prompt=custom_prompt, graph_model=graph_model, instruction_type=instruction_type
                )
...
async def save_interaction(data: str, instruction_type: str | None = None) -> list:
    """
    ...
    instruction_type : str, optional
        Embedding instruction preset to use while storing the interaction transcript.
        Select the preset that matches how the conversation will be queried later
        (qa, nl2code, code2code, code2nl, code2completion). Defaults to the global
        embedding setting when omitted.
    ...
    async def save_user_agent_interaction(data: str) -> None:
        ...
        with redirect_stdout(sys.stderr):
            ...
            await cognee_client.add(
                data, node_set=["user_agent_interaction"], instruction_type=instruction_type
            )

            try:
                await cognee_client.cognify(instruction_type=instruction_type)
...
@mcp.tool()
async def codify(repo_path: str, instruction_type: str | None = None) -> list:
    """
    ...
    instruction_type : str, optional
        Instruction preset for embedding code artifacts that come out of the code graph
        pipeline. Use `nl2code` when the agent will ask natural language questions about the repo,
        `code2code` for refactor/equivalence lookups, or `code2completion` when building
        autocompletion corpora. Defaults to the environment configuration when omitted.
    ...
    async def codify_task(repo_path: str):
        # NOTE: MCP uses stdout to communicate, we must redirect all output
        #       going to stdout ( like the print function ) to stderr.
        with instruction_scope(instruction_type), redirect_stdout(sys.stderr):
...
@mcp.tool()
async def search(
    search_query: str, search_type: str, instruction_type: str | None = None
) -> list:
    """
    ...
    instruction_type : str, optional
        Embedding prompt preset for both the query vector and any retrieved passages.
        Pick the template that matches the agent request:
        - `qa` for natural-language questions over documents
        - `nl2code` when the query is natural language but the corpus is code
        - `code2code` when both query and context are code
        - `code2nl` for comment/documentation synthesis
        - `code2completion` for completion seeds
        Leave unset to inherit the environment default.
    ...
    async def search_task(search_query: str, search_type: str) -> str:
        ...
        with redirect_stdout(sys.stderr):
            search_results = await cognee_client.search(
                query_text=search_query, query_type=search_type, instruction_type=instruction_type
            )
